...
過去に「別世代の Apps Script デプロイを叩いていた」ことが原因で管理画面がログイン不能になった
事故がある。**作業対象を間違えないよう、必ず Script ID で確認すること。**

| | Script ID（先頭） | 見分け方 |
| --- | --- | --- |
| 🟢 **本番（これを使う）** | `11ud1RV7…` | `/exec` に GET → `{"ok":true,"data":{"status":"ok","service":"himawari-hoop-court-gas"}}` |
| 🔴 旧・使用禁止「Hoop Court API」 | `1NsuHHQr…` | GET → `{"ok":true,"ping":"pong","tz":"Asia/Tokyo"}`（2026-07 にデプロイをアーカイブ済み） |

**コードによる見分け方**（エディタで検索する）:

- 本番 … `himawari-hoop-court-gas` を含む／未知 action に **`unsupported action:`** を返す
- 旧   … `ping: 'pong'` を含む／未知 action に **`unknown action:`** を返す

`/exec` URL の値は GitHub の `hoop-court` → Settings → Secrets and variables → Actions →
**Variables** の `NEXT_PUBLIC_GAS_ENDPOINT` が正。リポジトリにはコミットしない。

## この GAS を利用しているシステム（変更時の影響範囲）

| 利用者 | 用途 | エンドポイントの設定場所 |
| --- | --- | --- |
| `hoop-court`（LIFF 予約アプリ） | お客様の予約・マイページ | GitHub Variables `NEXT_PUBLIC_GAS_ENDPOINT` |
| `himawari-site`（管理画面） | 運営の予約管理・受付・売上 | GitHub Secrets `PUBLIC_GAS_ENDPOINT` |
...
**「新しいデプロイ」を作ってはいけない。** `/exec` URL が変わり、LIFF 予約アプリと管理画面が
**両方とも停止**する。

```
デプロイ → デプロイを管理 → 稼働中のウェブアプリの編集（鉛筆）
  → バージョン「新バージョン」→ デプロイ     （/exec URL は変わらない）
```

デプロイ後は `/exec` に GET してヘルスチェックが返ることを確認する。
問題があれば「デプロイを管理」から**以前のバージョンに戻せる**。
...
1. LINE Developers → **Messaging API チャネル**の「チャネルアクセストークン（長期）」を発行
2. Apps Script → プロジェクトの設定 → スクリプト プロパティに
   `LINE_MESSAGING_TOKEN` = そのトークン を追加
3. **再デプロイは不要**（プロパティは実行時に読まれる）
4. 翌日以降の枠でテスト予約を作成 → LINE に通知が届くことを確認 → テスト予約をキャンセル

> ⚠️ **前提: LINE ログインチャネルと Messaging API チャネルが同じプロバイダーに属していること。**
> 通知の宛先には、ログイン時の IDトークンから得た `userId` を使う。userId はプロバイダー単位で
> 一貫するため、別プロバイダーだと宛先が一致せず送信に失敗する。
> また、**公式アカウントを友だち追加していないお客様には届かない**（LINE 側が拒否する）。
...
| --- | --- |
| 予約確定通知 | 1予約につき **1通** |
| 一斉配信（`admin.broadcast`） | **友だちの人数分**（100人なら1回で100通） |

枠を圧迫する主因は一斉配信。予約通知を有効にする場合、配信は頻度と友だち数を確認してから行う。
超過分は送信されず、追加送信には有料プランへの変更が必要。
